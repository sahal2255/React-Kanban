    ...
    <div>
      <div className=" flex justify-between mx-5 border-solid rounded-lg border-2 border-sky-900 text-center mt-5 bg-slate-200 p-5">
        {statusCategories.map((category) => (
          <div key={category} className="w-1/3 border-dashed border-2 border-sky-500 p-4 bg-white rounded-lg shadow-lg mx-2">
            <h2 className="text-xl font-semibold mb-3">{category}</h2>
              <Droppable  droppableId={category} >
                {(provided) => (
                  <div ref={provided.innerRef} {...provided.droppableProps} className="space-y-3">
                    {tasks.filter((item) =>  item.status === category).map((task, index) => (
